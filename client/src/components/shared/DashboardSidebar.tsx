...
export default function DashboardSidebar({ role }: DashboardSidebarProps) {
  ...
  return (
    <aside className="w-80 bg-card/50 backdrop-blur-3xl border-r border-border h-screen sticky top-0 flex flex-col z-50 shadow-2xl">
      ...
      <nav className="flex-1 px-3 lg:px-6 space-y-1 lg:space-y-2 overflow-y-auto">
        ...
        {items.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}>
              <Button
                variant="ghost"
                className={`w-full justify-between h-12 lg:h-14 px-3 lg:px-5 rounded-xl lg:rounded-2xl transition-all duration-300 group ${
                  isActive 
